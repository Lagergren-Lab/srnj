...
Given medicc2 output on the 500 tumor cells selected from region E of the 10x breast cancer dataset, we first
...
def make_full_trees(C, A, cell_names, out_trees_dir, sparse_snv_matrix, clone_assignments):
    ...
    for method, tree in trees.items():
        ...
        f1_score = f1_score_clades(tree, clone_assignments.loc[cell_names])
...
def main():
    ...
    # load the medicc2 matrix
    D, rootdist, cell_names, rootlabel = get_medicc2_dist(medicc2_dist_path)
    C, A = get_lca_from_pairwise(D, rootdist)
    cell_names = [c.split('-', 1)[0] for c in cell_names]  # remove region prefix for matching with CHISEL

    print(f"Loaded medicc2 distance matrix with {C.shape[0]} cells and {C.shape[1]} distances.")
    ...
    make_full_trees(C, A, cell_names, out_trees_dir, sparse_snv_matrix, clone_assignments)
